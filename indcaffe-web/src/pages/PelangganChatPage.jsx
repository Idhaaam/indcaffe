...
const PelangganChatPage = () => {
    ...
    try {
      ...
      if (location.state?.newPartner) {
        ...
        if (np.productContext && !messages.length) {
            setNewMessage(`Halo, saya tertarik untuk membeli ${np.productContext.name}`);
        }
      } else {
        setPartners(fetchedPartners);
  ...
  return (
    <InternalLayout title="Chat Penjual">
        ...
        <div style={{ width: '320px', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', background: '#F8F9FA' }}>
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border-color)', background: 'white' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" className="form-input" placeholder="Cari kontak cafe..." style={{ paddingLeft: '36px', borderRadius: '20px' }} />
            </div>
          </div>
          
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {partners.length === 0 ? (
              <p style={{ padding: '20px', textAlign: 'center', color: 'var(--text-secondary)' }}>Belum ada kontak Cafe.</p>
            ) : (
              partners.map(p => (
                <div 
                  key={p.id}
                ...
                >
                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--accent-orange)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                    <User size={20} />
                  </div>
                  <div style={{ flex: 1 }}>
...
export default PelangganChatPage;
